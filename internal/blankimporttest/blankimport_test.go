...
// register themselves with the outbox plugin registry via init() side
// effects when blank-imported.
//
// This test lives in its own package because the plugin registry is a
// package-level singleton in outbox; tests inside the outbox package
// reset it for isolation. A separate test binary that blank-imports the
// plugins and observes the registry catches regressions in the
// registration mechanism itself — the kind of bug that would silently
...
func TestBlankImport_RegistersGcppubsub(t *testing.T) {
	names := outbox.RegisteredPlugins()
